...
GenerateRibbonsVInfectionVTransmission <- function(VI=0.8, VT=0.8, HouseholdSAR = 0.12){
  VaxPropDF = as_tibble(seq(0,100,2))|>rename(ProportionVaccinated = value)
  
  VaxPropDF|>
    mutate(tmp =  map(ProportionVaccinated/100,
                    ~CalcPopRvacRandom(probvac=.x,contacts = MSM, VI = VI, VT = VT, HouseholdSAR = HouseholdSAR))) |> 
    unnest(cols = c(tmp)) |> 
    mutate(Type = "Non-targeted") ->VaxPropPopRRandom 
  ...
  VaxPropDF|>
    mutate(tmp =  map(ProportionVaccinated/100,
                    ~CalcPopRvacTargetted(PropVac=.x,contacts = MSM, VI = VI, VT = VT, HouseholdSAR = HouseholdSAR))) |> 
    unnest(cols = c(tmp)) |> 
...
RibbonsVIVT |> 
  ggplot(aes(x = ProportionVaccinated))+
  geom_ribbon(aes(ymin = Lower, ymax = Upper,colour = Type,fill = Type), alpha = 0.4) +
  geom_hline(yintercept = 1,linewidth = 0.6, linetype = 'dotted')+
  facet_grid(VI~VT,labeller = 'label_both')+
  theme_bw() + 
  theme(legend.position="none",strip.background = element_rect(
...
ExtractVaxBands2 <- function(data = CladeIIb.df){
  data |> 
    select(ProportionVaccinated,Upper,Type) |> 
    slice_min(abs(1-Upper),n=1, by = 'Type') ->Upper
    # find the value of ProportionVaccinated where Upper crosses 1
  data |> 
    select(ProportionVaccinated,Lower,Type) |> 
    slice_min(abs(1-Lower),n=1, by = 'Type') ->Lower  
  
  left_join(Upper,Lower,by = 'Type') |> 
    select(Type, Upper = ProportionVaccinated.x,Lower = ProportionVaccinated.y)->tmp
  return(tmp |> mutate(ProportionVaccinated = 0))
}
  


GGPlotVaxRibbons = function(dataDF = RMSMCIIb.df){
  VaxBands = ExtractVaxBands2(dataDF)
  dataDF|>
    ggplot(aes(x=ProportionVaccinated,ymin = Lower,ymax=Upper,fill=Type))+
    geom_ribbon(alpha = 0.4) +
    geom_hline(yintercept=1,linetype='dotted')+
    geom_rect(data=VaxBands %>% filter(Type=='Non-targeted'),aes(xmin=Lower,xmax=Upper,ymin=0,ymax=Inf), colour = 'grey',alpha=0.1)+
    geom_rect(data=VaxBands %>% filter(Type=='Targeted'),aes(xmin=Lower,xmax=Upper,ymin=0,ymax=Inf),colour = 'grey',alpha=0.1)+
    ylab('Reproduction number')+
    xlab('% Vaccinated')+
    theme_bw()->p
  p
...
CladeIIb = GenerateRibbonsVInfectionVTransmission(HouseholdSAR = 0.013)

CladeIb = GenerateRibbonsVInfectionVTransmission(HouseholdSAR = 0.013 * 10)
...
GGPlotVaxRibbons(CladeIIb.df) -> Clade2Plot

GGPlotVaxRibbons(CladeIb) -> CladeIbPlot

GGPlotVaxRibbons(CladeI20) -> CladeIb20Plot

GGPlotVaxRibbons(CladeI40) -> CladeIb40Plot
  
GGPlotVaxRibbons(CladeIb50) -> CladeIb50Plot
...
HHSAR|>
  mutate(tmp = future_map(HouseholdSAR,
                          ~GenerateRibbonsVInfectionVTransmission(HouseholdSAR = .x),
                          .options = options)) |>
    unnest(tmp)-> RibbonPlotData

HHSAR|>
  mutate(tmp = future_map(HouseholdSAR,
                          ~GenerateRibbonsVInfectionVTransmission(HouseholdSAR = .x)|>ExtractVaxBands(),
                          .options = options)) |>
